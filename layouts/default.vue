...
<template>
    <div :class="{darkMode: darkMode}"  class="default duration-700 transition-colors w-screen h-screen flex flex-row-reverse">
        <div class="w-full md:pr-0 2xl:w-80 lg:w-64 md:w-36 md:fixed md:h-screen
     text-center flex flex-col md:flex-col overflow-y-auto">
            <div class="-mt-8 -mb-4 mx-4 md:m-0 hover:scale-105">
                <img v-if="darkMode === false" src="/TEST.png"/>
                <img v-if="darkMode" src="/TEST2.png"/>
            </div>
            <div class="text-3xl mt-10 mb-10">M-RAI</div>
            <label @change="toggle()" for="green-toggle" class="inline-flex self-center	 relative items-center mr-5 cursor-pointer">
...
<style>
    .default{
        background-color: rgba(29,42,41,255);
    }
    .darkMode{
        background-color: rgba(25,38,48,255);
    }
</style>